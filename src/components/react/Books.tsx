  ...
  return (
    <>
        ...
        <p className="flex flex-col space-y-2">
          <span className='text-zinc-200 font-[600] text-4xl'>Currently Reading</span>
          <span className='text-sm text-zinc-400'
            >Here is a list of all the books I'm currently reading. Yes, I'm a crazy person and read different books at the same time. Last updated: 10/08/2023</span>
      </p>

     
        <div className='flex flex-col space-y-3'> {CURRENTLY_READING.map((book, idx) => (
              <div
                key={idx}
                className='grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-zinc-300'
          >
             <a href={book.link} target='_blank' className='underline underline-offset-4 decoration-zinc-500 hover:decoration-zinc-200 duration-200'>        <span className='font-bold'>{book.title}</span></a>
        ...
        <p className="flex flex-col space-y-2">
          <span className='text-zinc-200 font-[600] text-4xl'>Finished Reading</span>
          <span className='text-sm text-zinc-400'
            >Here is a list of all <span className="font-bold text-zinc-200">+{numOfBooks}</span> of the books I've read. There are probably more books I've read, but I either forgot them, deleted my GoodReads before I could save them somewhere, and more excuses. Just trust me on it :) </span>
      </p>
        ...
        return (
          <div key={idx}>
            ...
            {booksForYear.map((book, idx) => (
              ...
              >
                <a href={book.link} target='_blank' className='underline underline-offset-4 decoration-zinc-500 hover:decoration-zinc-200 duration-200'>
<span className='font-bold'>{book.title}</span>
                ...
                <span className='text-yellow-600'>{'★'.repeat(book.rating)}</span>
              </div>
            ))}
          </div>
        )
